...
// =========================================================================
// TEST REVIEW SYSTEM - FOR DEVELOPMENT ONLY
// =========================================================================

/**
 * Create test reviews for testing the review system
 * 
 * Usage: Add ?create_test_reviews=1&seller_id=43 to any admin page URL
 * Security: Only works for logged-in administrators
 */
function create_test_reviews_for_seller() {
    // Security checks
    if (!is_admin() || !current_user_can('manage_options')) {
        return;
    }
    
    if (!isset($_GET['create_test_reviews']) || $_GET['create_test_reviews'] != '1') {
        return;
    }
    
    if (!isset($_GET['seller_id']) || !is_numeric($_GET['seller_id'])) {
        wp_die('Error: seller_id parameter is required and must be numeric.');
    }
    
    $seller_id = intval($_GET['seller_id']);
    $num_reviews = isset($_GET['num_reviews']) ? intval($_GET['num_reviews']) : 15;
    
    // Check if seller exists
    if (!get_userdata($seller_id)) {
        wp_die("Error: Seller with ID {$seller_id} not found.");
    }
    ...
    echo "<div style='max-width: 800px; margin: 20px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd;'>";
    echo "<h1>🧪 Creating Test Reviews</h1>";
    echo "<p><strong>Seller ID:</strong> {$seller_id}</p>";
    echo "<p><strong>Number of reviews:</strong> {$num_reviews}</p>";
    echo "<hr>";
    ...
    for ($i = 0; $i < $num_reviews; $i++) {
        ...
        if (!$user) {
            ...
            if (is_wp_error($user_id)) {
                echo "<p style='color: red;'>❌ Error creating user {$username}: " . $user_id->get_error_message() . "</p>";
                continue;
            }
            ...
            echo "<p style='color: blue;'>👤 Created test user: {$username} ({$user_data['first_name']} {$user_data['last_name']})</p>";
        } else {
            $user_id = $user->ID;
        ...
        if ($result['success']) {
            ...
            $created_count++;
            echo "<p style='color: green;'>✅ Created review #{$created_count} - {$review_data['rating']} stars by {$user_data['first_name']}</p>";
        } else {
            echo "<p style='color: red;'>❌ Error creating review: {$result['message']}</p>";
        }
    }
    ...
    echo "<div style='background: #e7f3ff; padding: 15px; margin: 15px 0; border-left: 4px solid #2196F3;'>";
    echo "<h3>🎯 Next Steps:</h3>";
    echo "<ul>";
    echo "<li>Visit your seller profile page to see the reviews</li>";
    echo "<li>Check the admin dashboard: <a href='" . admin_url('admin.php?page=seller-reviews') . "'>Seller Reviews</a></li>";
    ...
    echo "<p><strong>⚠️ Development Only:</strong> This function should be removed before going live!</p>";
    echo "</div>";
    
    echo "</div>";
    
    // Stop execution so we only see the results
    wp_die();
}
add_action('admin_init', 'create_test_reviews_for_seller');