...
// =========================================================================
// BULK CAR LISTING CREATOR - UPDATED FOR NEW JSON STRUCTURE
// =========================================================================
...
function bulk_create_car_listings() {
    ...
    // Load car makes data from JSON files - NEW FORMAT
    $makes_data = [];
    ...
    if (is_dir($jsons_dir)) {
        ...
        foreach ($json_files as $file) {
            ...
            // NEW JSON STRUCTURE: {"make": "MAKE_NAME", "models": [...]}
            if ($data && isset($data['make']) && isset($data['models'])) {
                $make_name = $data['make'];
                $models = $data['models'];
                if ($make_name && is_array($models)) {
                    $makes_data[$make_name] = $models;
                }
            }
        }
    }
    ...
    for ($i = $start_index; $i <= $end_index; $i++) {
        ...
        // Select random make and model with error handling - UPDATED FOR NEW STRUCTURE
        try {
            ...
            // Models are now just a simple array of strings
            $models = $makes_data[$random_make];
        ...
        // Create post title and description - NO VARIANT FIELD
        $post_title = "$year $random_make $random_model";
        $description = "Beautiful $year $random_make $random_model in excellent condition. " .
        ...
        // Add all the ACF fields (like manual submissions) - VARIANT FIELD REMOVED
