    ...
    function setCookieConsent(type, preferences = {}) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    if (window.isDevelopment) console.log('Cookie preferences saved:', response.data.preferences);
                    ...
                    if (window.isDevelopment) console.error('Failed to save cookie preferences');
                }
            },
            error: function(xhr, status, error) {
                if (window.isDevelopment) console.error('AJAX error:', error);
            }
        });
    }
    ...
    function getCookiePreferences() {
        ...
        if (cookie) {
            try {
                ...
                if (window.isDevelopment) console.error('Error parsing cookie preferences:', e);
                // Try parsing without decoding in case it's not URL encoded
                try {
                    return JSON.parse(cookie);
                } catch (e2) {
                    if (window.isDevelopment) console.error('Error parsing cookie preferences (second attempt):', e2);
                    return null;
                }
            }
        }
