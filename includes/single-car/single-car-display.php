...
if (have_posts()) :
    while (have_posts()) : the_post();
        ...
        if (get_post_type($car_id) === 'car') :
            ...
            <?php
            // Add the image data to the page for JavaScript
            $image_urls = array();
            foreach ($all_images as $image_id) {
                $image_urls[] = wp_get_attachment_image_url($image_id, 'large');
            }
            ?>
            <script>
                window.allImagesData = <?php echo json_encode($image_urls); ?>;
            </script>
