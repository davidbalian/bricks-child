<?php
/**
 * Buyer Request Call Button Click Tracking
 * 
 * Tracks call button clicks for buyer request posts.
 * 
 ...
 */
function handle_buyer_request_call_button_click() {
    // Verify nonce for security
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'buyer_request_call_button_click')) {
    ...
    $current_clicks = get_field('buyer_call_number_click', $post_id);
    ...
    $updated = update_field('buyer_call_number_click', $new_count, $post_id);
...
add_action('wp_ajax_buyer_request_call_button_click', 'handle_buyer_request_call_button_click');
add_action('wp_ajax_nopriv_buyer_request_call_button_click', 'handle_buyer_request_call_button_click');

/**
 * Enqueue JavaScript for call button click tracking
 */
function buyer_request_call_button_enqueue_scripts() {
    if (is_singular('buyer_request')) {
        ...
        window.location.search.includes('debug=true')
            ...
            $('.buyer-request-call-button, .meta-phone-link').on('click', function(e) {
                var button = $(this);
