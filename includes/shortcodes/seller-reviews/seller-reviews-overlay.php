...
function seller_reviews_overlay_shortcode($atts) {
    ...
    $atts = shortcode_atts(array(
        'seller_id' => '',
        'show_compact' => 'true', // Show the compact stars display
    ), $atts, 'seller_reviews_overlay');
    
    // Enqueue necessary styles and scripts
    $theme_dir = get_stylesheet_directory_uri();
    
    if (!wp_style_is('seller-reviews-overlay', 'enqueued')) {
    ...
    ob_start();
    ?>
    <div class="seller-reviews-complete-container" data-seller-id="<?php echo esc_attr($seller_id); ?>">
        
        <?php if ($atts['show_compact'] === 'true'): ?>
        <!-- Compact Stars Display -->
        <div class="seller-rating-compact">
            ...
            <?php if ($reviews_summary['count'] > 0): ?>
                <button type="button" class="btn-show-reviews" data-seller-id="<?php echo esc_attr($seller_id); ?>">
                    See all reviews
                </button>
            <?php endif; ?>
        </div>
        ...
        <!-- Reviews Overlay (hidden by default) -->
