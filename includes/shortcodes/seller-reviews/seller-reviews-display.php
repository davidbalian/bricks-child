...
function seller_reviews_display_shortcode($atts) {
    ...
    if (!wp_style_is('seller-reviews-display', 'enqueued')) {
        ...
        wp_localize_script('seller-reviews-overlay', 'sellerReviewsData', array(
            ...
            'strings' => array(
                'verifyEmail' => __('Please verify your email before leaving a review.', 'bricks-child'),
                'selectRating' => __('Please select a rating.', 'bricks-child'),
                'submitting' => __('Submitting...', 'bricks-child'),
                'genericError' => __('An error occurred. Please try again.', 'bricks-child'),
                'limitExceeded' => __('Character limit exceeded by %s characters', 'bricks-child'),
                'charactersRemaining' => __('%s characters remaining', 'bricks-child'),
            ),
        ));
    }
    ...
    if (empty($seller_id)) {
        return '<p>' . esc_html__('No seller specified.', 'bricks-child') . '</p>';
    }
    ...
    <div class="seller-reviews-container <?php echo esc_attr($container_mod); ?>" data-seller-id="<?php echo esc_attr($seller_id); ?>">
        ...
        <div class="seller-reviews-list">
            <h4><?php esc_html_e('Recent Reviews', 'bricks-child'); ?></h4>
                    ...
                    <div class="review-header">
                        ...
                        <div class="review-meta">
                            ...
                            <?php if ($review->contacted_seller): ?>
                                <span class="contacted-badge"><?php esc_html_e('Contacted Seller', 'bricks-child'); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <div class="seller-reviews-overlay" style="display: none;">
        <div class="seller-reviews-overlay-content" data-seller-id="<?php echo esc_attr($seller_id); ?>">
            ...
            <div class="overlay-header">
                <h3><?php echo esc_html(sprintf(__('Reviews for %s', 'bricks-child'), get_userdata($seller_id)->display_name)); ?></h3>
                ...
                <div class="rating-display">
                    ...
                    <span class="rating-text">
                        <?php if ($reviews_summary['count'] > 0): ?>
                            <?php echo esc_html(sprintf(__('%s out of 5', 'bricks-child'), number_format($reviews_summary['average'], 1))); ?>
                            (<?php echo $reviews_summary['count']; ?>
                            <?php echo _n('review', 'reviews', $reviews_summary['count'], 'bricks-child'); ?>)
                        <?php else: ?>
                            <?php esc_html_e('No reviews yet - be the first to review!', 'bricks-child'); ?>
                        <?php endif; ?>
                    </span>
                </div>
            </div>
            ...
            <div class="overlay-reviews-section">
                ...
                if (!empty($all_reviews)): ?>
                    <h4><?php esc_html_e('All Reviews', 'bricks-child'); ?></h4>
                        ...
                        <?php foreach ($all_reviews as $review): ?>
                            <div class="review-item">
                                <div class="review-header">
                                    ...
                                    <div class="review-meta">
                                        <span class="reviewer-name">
                                            <?php echo esc_html($review->reviewer_name ?: __('Anonymous', 'bricks-child')); ?>
                                        </span>
                                        <span class="review-date">
                                            <?php echo esc_html(date_i18n('F j, Y', strtotime($review->review_date))); ?>
                                        </span>
                                        <?php if ($review->contacted_seller): ?>
                                            <span class="contacted-badge"><?php esc_html_e('Contacted Seller', 'bricks-child'); ?></span>
                                        <?php endif; ?>
                ...
                <?php else: ?>
                    <div class="no-reviews">
                        <p><?php esc_html_e('No reviews yet for this seller.', 'bricks-child'); ?></p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="overlay-review-form-section">
                ...
                <?php if (get_current_user_id() == $seller_id): ?>
                    <div class="review-notice">
                        <p><?php esc_html_e('You cannot review yourself.', 'bricks-child'); ?></p>
                    </div>
                <?php elseif ($strict_reviews && ! is_user_logged_in()): ?>
                    <div class="login-prompt">
                        <p>
                            <a href="<?php echo esc_url(wp_login_url(get_permalink())); ?>" class="login-link"><?php esc_html_e('Log in', 'bricks-child'); ?></a>
                            <?php esc_html_e('to leave a review for this seller.', 'bricks-child'); ?>
                        </p>
                    </div>
